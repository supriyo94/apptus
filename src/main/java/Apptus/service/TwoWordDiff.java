...
@Component
public class TwoWordDiff {
    public ResponseDTO WordDiffernces(String path1,String path2) {
        ResponseDTO responseDTO=new ResponseDTO();
        Map summaryText = null;
        try {
            int deleteCount = 0;
            int insertCount = 0;
            int numberChange = 0;
            summaryText = new HashMap();
            // Load the first Word document
            //XWPFDocument doc1 = new XWPFDocument(new FileInputStream("v1.docx"));
            XWPFDocument doc1 = new XWPFDocument(new FileInputStream(path1));
            // Load the second Word document
            //XWPFDocument doc2 = new XWPFDocument(new FileInputStream("v2.docx"));
            ...
            summaryText.put("deleteCount", deleteCount);
            summaryText.put("insertCount", insertCount);
            summaryText.put("numberOfChange", numberChange);
            summaryText.put("insertedTexts", insertedTexts);
            summaryText.put("deletedTexts", deletedTexts);
   ...
   // Comparator<Map.Entry<String, String>> byNumberComparator = Comparator.comparingInt(o -> Integer.parseInt(o.getKey()));
